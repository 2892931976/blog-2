...
'''
if system() == 'Windows':
    log_path = r"C:\blog\%s_%s"%(current_time, pid)
else:
    log_path = "/var/log/blog/%s_%s"%(current_time, pid)
'''

if system() == 'Windows':
    log_path = r"C:\blog\tornado.log" 
else:
    log_path = "/var/log/blog/tornado.log" 
...
def init(console_handler=True, file_handler=True, log_path=FILE['log_path'], base_level="INFO"):
    try:
        if system() == 'Windows':
            os.makedirs(r"C:\blog")
        else:
            os.makedirs(r"/var/log/blog")
    except:
        pass
